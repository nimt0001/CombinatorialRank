########################## KM Analysis new for Frontier Oncology paper - ALDH1A2
library(survival)
library(MASS)
library(cgdsr)
library(HapEstXXR)
...
mycancerstudy = getCancerStudies(mycgds)[118,1] 
...
mygeneticprofile_exp_z = getGeneticProfiles(mycgds,mycancerstudy)[3,1] 
...
# Plot the KM curve for any subset
my_event_rna = my_event_rna_list[[sample(1:length(ps),1)]] #pick a random subset
s_temp <- Surv(as.numeric(as.character(all_clin$new_dfsMonths)),all_clin$dfs_events)
s_new <- survfit(s_temp~my_event_rna)  
s1_new <- tryCatch(survdiff(s_temp~my_event_rna), error = function(e) return(NA))
pv_new <- ifelse(is.na(s1_new),next,(round(1 - pchisq(s1_new$chisq, length(s1_new$n) - 1),3)))[[1]]
...
# Cox regression for any gene subset
prostate <- c()
